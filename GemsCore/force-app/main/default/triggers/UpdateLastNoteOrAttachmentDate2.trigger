trigger UpdateLastNoteOrAttachmentDate2 on Attachment (after insert, after update) {
    ...
    for (Attachment a : trigger.new){
        ...
        if (parentIdString.substring(0,3) == '006'){
        	opptyIds.add(a.parentId);
            Opportunity oppUpdate=new Opportunity(id=a.parentId);
            oppUpdate.Last_Note_or_Attachment_Date__c = System.now();
            mapUpdateOpportunity.put(a.parentId,oppUpdate);      
        }        
    }
